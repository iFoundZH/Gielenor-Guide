...
Gielinor Guide is a **DPS Calculator** for the OSRS Demonic Pacts League (live Apr 15–Jun 10 2026). Implements exact OSRS combat formulas, all ~130 pact modifiers, a smart gear optimizer, boss presets, and 10 echo items with wiki-verified stats. Purely client-side with no backend; builds are saved in localStorage.
...
**Playwright E2E tests** in `e2e/` directory — 36 tests across 5 spec files, Chromium only.
...
- `src/components/calculator/` — Calculator-specific: PlayerConfigPanel, BossSelector, RegionSelector, PactSelector, GearGrid, ItemPicker, DpsResultCard, DpsBreakdown, TopBuildsPanel
- `src/data/` — Static data: `items.ts` (~120 items), `pacts.ts` (~50 pacts), `boss-presets.ts` (~30 bosses)
- `src/lib/` — Core logic: `dps-engine.ts`, `gear-optimizer.ts`, `build-storage.ts`
...
- **Pact modifiers** are DP-league specific with exact multiplier values.
...
- Leagues VI. Separate servers, boosted XP/drops, pact system.
- Pacts: toggleable modifiers that boost and penalize. ~130 total, ~25 DPS-relevant.
